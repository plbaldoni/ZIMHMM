...
ChIP-seq experimental replicates. `ZIMHMM()` models the zero-inflation
of background counts (ZI), accounts for replicate-specific differences
via a mixed effects model (M), and ensures that broad regions of
enrichment are detected by fitting a hidden Markov model (HMM). This
package also contains `ZIHMM()`, a fixed effects version of the peak
caller. The function `controlPeaks()` provides parameters that control
